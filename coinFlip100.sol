...
contract CoinFlip is Ownable{
    ...
    function verifyBet(uint[] memory betNumbers, uint[] memory betAmount_wei) private view returns(bool) {
        ...
        if (betNumbers.length != betAmount_wei.length) {
            return false;
        }
        uint sumBetAmount = 0;
        ...
        for (uint x = 0; x<betNumbers.length; x++) {
            ...
            if (betNumbers[x] < BET_RANGE_MIN || betNumbers[x] > BET_RANGE_MAX) {
                return false;
            }

            if (betAmount_wei[x] <  BET_MIN_AMOUNT_WEI) {
                return false;
            }
            sumBetAmount += betAmount_wei[x];
        }

        if (sumBetAmount > playerPlayableFund()) {
            return false;
        }
        return true;
    }
